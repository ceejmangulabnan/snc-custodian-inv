...
import { readRefreshTokenFromHeaders } from '../../utils/strapi'
...
function isStrapiError(body: unknown): body is { error?: { message?: string, detail?: string } } {
...
export default defineEventHandler(async (event) => {
  ...
  const loginResponse = await $fetch.raw<StrapiLoginResponse>(`${strapiUrl}/api/auth/local`, {
    method: 'POST',
  ...
  if (loginResponse.status !== 200 || !loginResponse._data?.jwt || !loginResponse._data?.user) {
  ...
  const meResponse = await $fetch.raw<StrapiMeResponse>(`${strapiUrl}/api/users/me`, {
    query: { populate: 'role' },
